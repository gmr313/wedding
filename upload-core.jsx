...
const CLOUDINARY_FOLDER = "wedding"; // top-level folder; each guest gets a subfolder by their name
//
...
async function uploadItem(item, guestName, onProgress) {
  ...
  if (STORAGE === "cloudinary") {
    ...
    // Put each guest's uploads into their own subfolder, e.g.
    // "wedding/Jane Smith". Guests who skip the name go to "wedding/_no name".
    const safeName = (guestName || "").trim().replace(/[\\/:*?"<>|]+/g, " ").replace(/\s+/g, " ").slice(0, 60);
    form.append("folder", CLOUDINARY_FOLDER + "/" + (safeName || "_no name"));
